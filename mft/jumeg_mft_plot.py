...
def plot_global_cdv_dist(stcdata,fwdmag=None):
    ...
    if fwdmag is None:
        ...
        for isrc in xrange(len(fwdmag['src'])):
            cdvnmax = stcdata[offsets[isrc]:offsets[isrc+1], time_idx]
            plt.plot(xrange(offsets[isrc],offsets[isrc+1]), cdvnmax)
...
def plot_cdm_data(qualmft,stc_mft, cdmlabels=None, selmaxjlong=False, outfile=None):
        ...
        for ilab, label in enumerate(cdmlabels):
            if icdmtop[ilab] > 0:
                ...
                if label.color is not None:
                    plt.plot(1e3 * stc_mft.times, qualmft['cdmlabels'][ilab,:], color=label.color, label="%2d" % (ilab+1))
                else:
    ...
    if not qualmft.has_key('cdmall') and not qualmft.has_key('cdmright') and \
       not qualmft.has_key('cdmleft'):
        return
    print "##### Attempting to plot cdm data:"
    fig = plt.figure()
    plt.ylim((0,1.05))
    if qualmft.has_key('cdmleft'):
        plt.plot(1e3 * stc_mft.times, qualmft['cdmleft'][:], 'r', label='lh')
    if qualmft.has_key('cdmright'):
        plt.plot(1e3 * stc_mft.times, qualmft['cdmright'][:], 'g', label='rh')
    if qualmft.has_key('cdmall'):
...
def plot_jlong_labeldata(qualmft,stc_mft, cdmlabels, outfile=None):
    ...
    for ilab, label in enumerate(cdmlabels):
        if ijlgtop[ilab] > 0:
            ...
            if label.color is not None:
                plt.plot(1e3 * stc_mft.times, qualmft['jlglabels'][ilab,:], color=label.color, label="%2d" % (ilab+1))
            else:
...
def plot_jtotal_labeldata(qualmft,stc_mft, cdmlabels, outfile=None):
    ...
    for ilab, label in enumerate(cdmlabels):
        if ijtottop[ilab] > 0:
            ...
            if label.color is not None:
                plt.plot(1e3 * stc_mft.times, qualmft['jtotlabels'][ilab,:], color=label.color, label="%2d" % (ilab+1))
            else:
